...
\item {Jellibeans saves the user's Autism score for the next time they sign in, so that this information can be used to evaluate the search tool (i.e., see if it assists users with Autism form search queries. This feature will be implemented using MySQL and a database to store the records.}
...
\vspace{5mm}
This means that Jellibeans is successfully reducing the number of results the participant has to sift through, without reducing precision of the search tool.
