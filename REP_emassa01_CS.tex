...
As search can be compartmentalised into `Do', `Know' and `Go' type queries, it provides a natural way to takle the issue of simplifying the search process for people with Autism. If users first identify the type of search they would like to carry out, the search engine can better phrase a question to guide the user through their informational demand. Each stage of the search process described above, can be better tailored towards providing a concrete and unambiguous experience for the user with Autism. For example, using Jellibeans, at the query formulation stage, the user will be assisted through the process of formulating a search query, with concrete questions and suggestions being offered. The suggestions will act as cues, and will be useful to simplify the search process. The process of forming a query will be more tightly defined than the current systems for web search available, and the process more predictable.  

wanted to see if combination search engine enhanced the search process or not
whether a parametric search was useful
whether reducing results back to the user was a good idea
...
\caption{Suggestions on Jellibeans `Go' page for users to help with search query formulation.}
\label{suggestions}
\end{center}
\end{figure}
