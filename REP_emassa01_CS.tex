...
\end{center}
\caption{Wireframe Sketches for Jellibeans User Flow. Jellibeans can be found at http://esha.mseth.co}
\label{JBeanUserFlow}
\end{figure}
...
\item{Rendering the Application to the Web Browser (HTML, CSS, BootStrap).}
\subitem{Integration with Google+ API (JavaScript).}
...
\subitem{The Data Store Module works with the Google Custom Search engine to retrieve the (indexed) documents from the data store. The documents that are retrieved match the participants' search query string after processing at the logic module level. }
\end{itemize}
...
\end{center}
\end{figure}
A class diagram of the system can be seen in Figure~\ref{jBeanClassDiagram}.
...
\subsection{APIs Used in Jellibean Implementation}\label{implementation}
