...
\begin{itemize}
\item{The \textbf{Model}- is the domain or real world entity that the software is built around. The models in this case are:}
\subitem{The User, and}
\subitem{The User's Search Queries.}
...
The Autism Quotient score and all 50 of the responses from the user will be stored on a database (MySQL). The database was specified and created using several laravel migration files. Migrations are paired with Laravel's schema builder to build the Jellibeans database schema (see Figure~\ref{usertable}, Figure~\ref{passwordtable}, Figure~\ref{querytable}).


\begin{figure}[H]
\begin{center}
\includegraphics[scale=0.5]{userTable}
...
\begin{figure}[H]
\begin{center}
\includegraphics[scale=0.5]{passwordTable}
...
\begin{center}
\includegraphics[scale=0.5]{passwordTable}
\caption{User Passwords Model Migration (Database Table).}
\label{querytable}
\end{center}
\end{figure}
\begin{figure}[H]
...
\caption{Recall for Jellibeans versus the usual participant search tool (for each participant).}
\label{result2}
\end{center}
\end{figure}
