...
\section*{Abstract}
\begin{justify}
This project report presents my research and development of a prototype web application to assist users with Autism when they search the web. The system models user interactions with the search process into a user profile for people with Autism, integrating insights from the core features of to condition into the model. The system is integrated with an infra-red, motion controlled, user interface component to assist users with Autism during search. The project provides insights into how search can be improved for users with Autism.\\
\end{justify}
\begin{verbatim}
...
\begin{center}
word count (project text only) : 13357 words
\end{center}
...
\vspace{5mm}
Jellibeans is a dynamic website (written using a server-side scripting language) and implements a combination search. This means it synthesises the results from three of the largest and most popular engines; Google, Bing and Yahoo \cite{adam}.

\vspace{5mm}
Jellibeans (jellibeans.mseth.co) allows the user to log in and out. User diagnostic information is persisted to a SQL database. Each user has a unique ID number and a password. Prototype versions of Jellibeans (esha.mseth.co) experimented integrating a Google+ sign in to Jellibeans so that users can have their profile picture and name displayed. Both sign-in functionalities enhance the feel of a personalised search engine (see Figure~\ref{googlesignin}).
...
A client-server software architecture was used for Jellibeans. The presentation layer consisted of the user interface and motion controller. The presentation layer, in turn, has access to the application layer, which controls the functionality of the system by performing detailed processing. This included management of Jellibeans within the \textbf{Laravel} PHP web-application framework. 

\vspace{5mm}
Laravel is a PAAS (platform as a service), which manages the deployment of Jellibeans to a \textbf{Digital Ocean} Cloud Server. Laravel implements a \textit{rewrite engine}, which is software that modifies the URLs appearance so that the URL is shorter and more informative/relevant for the user. The technique loosens the coupling between the HTML files and the URL that is presented to the user.

\begin{itemize}
\item{The application layer therefore included (amongst other things):}
\subitem{The php routing code. The routing code allowed the URI (Uniform Resource Identifier) to be broken up into parameters and processed to determine which view (presentation/html file/web page) to present to the user.}
\subitem{The JavaScript code used to implement the custom search.}
\subitem{The SQL execution code for the storage of data in the data store.}
\end{itemize}
...
\begin{figure}[H]
\begin{center}
\includegraphics[scale=0.55]{ntier}
...
\subitem{Jellibeans Web Application: Rendering the web application to the browser (HTML, CSS PHP and Bootstrap).}

\subitem{Rendering a Parametric-Like Search in the browser: The user is asked specific questions to break down their search query into smaller manageable terms, i.e., into `Do', `Know', `Go', or `Social' queries, and then into direct search terms. This reduces the ambiguity individuals with Autism face when forming a search query. The web application provides suggestions or cues for users. Once the search terms are entered by the user, they are sent to the logic module to form the search query string, and then onto the data store to retrieve the results which are subsequently rendered onto the screen for the user (uses HTML, CSS, php and Bootstrap).}

\subitem{Google+ Integration: Jellibeans integrates with Google+ API to retrive and render user profile data. This enhances the feel of a personalised search engine. Once signed in, users complete a diagnostic assessment of Autism (the Autism Quotient \cite{Baron Cohen et al}). The user's entries are sent to the logic module to be calculated. A user can also save a copy of their diagnostic information to a local file on their computer (uses JQuery, JavaScript and Google+ API).}

\subitem{The user's AQ score is saved to a SQL database once the score is calculated. The score is fetched from the database whenever the user logs in. A unique identification number assures that the user data is never overwritten or duplicated.}

\subitem{LEAP UI Component: Jellibeans is a LEAP enabled website, and has a Motion Controlled UI in the web browser. The motion controller provides an interactive search experience, and maintains the user's full attention during search (uses LEAP SDK and LeapStrap).}

\subitem{Importantly - the Presentation module includes the routing logic to determine the view (or web page) to present to the user.}

\item{Logic Module\\
At the logic layer, Jellibeans works to build user search queries from the elements on the web page. The user's search-parameters are entered when they form their search query at the presentation layer. \\
The Logic module enables:}

\subitem{Logic for the user to carry out a single search that retrieves results from a combination of search engines. The results returned are those synthesised from Google, Bing and Yahoo. The aim of this feature is to increase recall of the search engine by increasing the number of potential relevant results (programmed into a Java applet).}

\subitem{Logic to integrate with the Google+ API (uses JavaScript and Google+ API).}

\subitem{Calculation of the users' AQ Score using the questionnaire entries that are entered at the presentation layer. The request is sent to the database (MySQL). The score is returned to the presentation layer after calculation ( uses JavaScript).}

\subitem{Logic to formulate the user's search query, given their entries at the presentation layer (uses JavaScript).}

\subitem{Logic to send the query to the Google Data Store, and, to retrieve the search results to render them to the presentation layer (uses JavaScript and Google Custom Search API).}

\subitem{Logic to prioritise the three highest ranking search results and to reduce the text and verbal information on the page (in line with the aims of Jellibean Search), (uses JavaScript and Google Custom Search API).} 
...
\subitem{Jellibeans stores and retrieves the user's Autism score each time the user signs in from a database (uses MySQL). }
...
\item{The \textbf{View}- is the visual representation of the user, given a context. So, in this case it is the resulting markup that the php framework (Laravel) renders to the users browser (i.e., the html code). The view generates the user interface, and is accessible via the user (the Model). The view doesn't handle the data that comes in (although users input the data into the view that is presented to them on the application's webpage). There are 6 views of the system.}
...
\item{The \textbf{Controller}- Coordinators provide links between the view and the model. Coordinators are responsible for input-processing and determining the behaviour of Jellibeans, such as which new screen to render. Controller.php is an abstract class that all other controllers extend (see Figure~\ref{controller}). Jellibeans has the following Controllers:}
...
\subitem{SessionsController.php controls the behaviour of the view depending on whether the user is logged in or logged out. For example, if the user is logged out it will display the register and login buttons on the html page. If the user is logged in, it will return the logout button. }
...
\begin{figure}[H]
\begin{center}
\includegraphics[scale=0.70]{googlesignin2}
\end{center}
...
\subsection{Building a User Model of Autism.}\label{requirements}
...
There was a lot more variation in the responses gathered from the group with Autistic-like traits. The responses can be seen in Section~\ref{asdresponses}. The following observations were made for the users in the high AQ group:
\begin{enumerate}
...
The calculated Autism Quotient score from the user will be stored on a MySQL database system. The database configuration for the system was specified in a configuration php file  (see Figure~\ref{dbconfig}).
...
To test the integrated combined search engine, 10 participants took part in a study to evaluate the quality of it's output. The participants were selected because they scored highly on a 50-item screening measure of Autism traits (the Autism Quotient, or AQ \cite{Baron Cohen et al}). The range of scores for the AQ is 0 to 50 with high scores indicating increased likelihood of autism-like traits. A score under 21 is a low to average result (many women average around 15 and men around 17). A score of 22-25 indicates autistic tendencies slightly above the population average. A score above 26 gives a borderline indication of high functioning autism, or Aspergers. A score above 30 suggests a likelihood of Aspergers syndrome or autism (sensitivity of test measure = 79\% \cite{Baron Cohen et al}). For the purposes of this study, individuals with scores equal to, and above 30 were interpreted as having `autistic-like traits', and were selected to take part as these individuals were thought to be most suited to test the current system. 

\vspace{5mm}
Participants were asked to comment on their search results from 10 informational needs. Participants were also asked to choose three out of the links returned that they would choose to follow up with. They were also asked if anything was odd about the results returned. The responses from the 10 users were then analysed. 
...
\begin{comment}
You could expand on e.g.: 
a) the kind of BDD cases you have used to drive your development (maybe providing an example), 
b) the tools you have used for BDD (Cucumber? FITNESSE? others?), 
c) the kind of automated testing that you have put in place for your Javascript code (assuming you have some, it is unclear in the
current writing),  JSLint (http://www.jslint.com/)
d) the number of lines of code dedicated to testing (at the very least, you mention you are using JUnit), and the testing
coverage of your code. Do not be scared of admitting that your coverage is under 95\% (if that is the case): it is better to admit a
coverage of 50\% (or 30\%, or whatever) than saying nothing and letting the evaluators assume a low coverage of 10\%-15\%. 
...
Behaviour Driven Development (BDD) was used in the current project. This is an agile technique, which includes elements of user acceptance testing and customer test driven development. This was achieved by obtaining a clear understanding of the desired behaviour of the application through discussion with the stakeholders (users with autism) during the \textit{Requirements Gathering} phase of the study (see Section~\ref{requirements}). During the requirements gathering phase, the purpose and benefit of each feature was reviewed and evaluated. In this way the user with Autism (or stakeholder) participated in the discussion and design of Jellibeans (or the user model used within Jellibeans) which added significant value to the application from the user's perspective. 

\vspace{5mm}
One case example in which BDD was applied was in the development of the parametric search tool ( search tool presented after users indicate whether they are doing a `Do', `Know', `Go' or `Social' search). The first practice of BDD is to establish the goals of the different stakeholders. One goal for the application itself, was to increase the number of search terms being used by people with Autism, whilst the goal for the user was to accurately define their search query and retrieve relevant search results. The feature that was established to achieve both these goals was a parametric search tool, which was designed to be more complex and intricate for search queries that were identified as particularly difficult for people with Autism, for example, if the user had identified that they were specifically conducting a `Social' search.

\vspace{5mm}
The BDD process includes assuring that the software performs the way in which it is expected to, at all times. BDD was used to test the integration of the HTML, CSS and JavaScript code in Jellibeans. The testing process consisted of development, test, make modifications, test (and so on) until the behaviour of the website was what was intended. The entire set of requirements identified in Section ~\ref{requirements} were used to drive the development of the web application. 

\vspace{5mm}
The \textbf{FitNesse} framework for integrated testing was used to test the software collaboration functionality of the web application. It is open source software \cite{fitnesse}, and was chosen over Cucumber because the developer felt it was extremely interactive and transparent. Unlike unit testing (which tests efficiency of code), FitNesse was used in a browser GUI localhost to present human-readable tests to test the value of a feature on the application. 

\vspace{5mm}
Unit testing was also compulsory to ensure the code was efficient and complemented the value of the features identified during BDD. The JUnit testing framework was used to test both the Combination Search and LEAP controller methods. The code to implement the Combination Search Engine and LEAP motion controller methods was written using Java (the combination search was presented to the user in an applet). For the Combination Search, Unit testing was conducted for the number and accuracy of search results from each search engine (Google, Yahoo and Bing). The results gathered from the applet, were also confirmed in real time, against a live search using the developers machine. For the LEAP controller methods, Test Driven Development Unit testing was completed, at the level of each method implemented. Over 85\% of the Java code implemented in the current project was supported by Unit testing. The remaining 15\% was not tested due to the variable nature of some search engines, returning slightly different results on each iteration of a web search (and so, as explained above, in these cases the results of the web search were validated in real time).

\vspace{5mm}
Manual testing for JavaScript included working with the JavaScript Console  \cite{javascripttesting}, I also used JSfiddle \cite{jsfiddle}. This was particularly useful to ensure the code was error-free. These tools also enabled me to see how well the JavaScript code integrated with the HTML code and CSS. JSLint  \cite{jslint} was another useful JavaScript testing tool used in the current project, to solely test JavaScript code. The program scans JavaScript code looking for errors, and when it finds one, it returns a message and location of the error in the code.

\vspace{5mm}
After manual verification was satisfied, automated verification of the JavaScript tests were run using QUnit \cite{qunit}. QUnit is a JavaScript Unit test framework and was used to automate testing JavaScript code for the current project, including some JQuery.

\vspace{5mm}
Lastly, regression tests were run after unit testing, including testing Jellibeans on users with Autism to evaluate the efficiency of the system as a whole. This next phase of testing is considered in Section ~\ref{usertesting} of this report.
...
\section{User Verification and User Testing} \label{usertesting}
...
\bibitem{bcrypt} Provos, Niels, Mazières, David and Talan Jason Sutton (1999). \textit{A Future-Adaptable Password Scheme}. Proceedings of 1999 USENIX Annual Technical Conference: 81–92.
...
\bibitem{filhidtable} Data Store Using HTML, http://stackoverflow.com/questions/27773788/how-do-i-store-data-from-an-offline-form-using-just-html-or-javascript, Retrieved July 1 2015.
...
\bibitem{fitnesse} FitNesse, http://www.fitnesse.org/, Retrieved 14 September 2015.
...
\bibitem{jslint} JSLint, http://www.jslint.com/, Retrieved 13 September 2015.
...
\bibitem{leapide} Huppert, N. (2015) https://github.com/Noah-Huppert/Leap-Motion/blob/master/MouseFinal, Retrieved August 10 2015.
...
\bibitem{qunit} QUnit, https://qunitjs.com/, Retrieved 14 September 2015.
...
\section{Languages and APIs Used in Jellibean Implementation}\label{implementation}
...
\newpage
\section{Google Terms of Servive} \label{GoogleToS1}
\begin{figure}[H]
\begin{center}
...
\newpage
\section{Search Query Survey} \label{AppendixA}
...
\newpage
\section{Data Coding for Result 1 and 2}\label{datacoding}
...
\newpage
\section{Questions on the Autism Spectrum Quotient \cite{Baron Cohen et al}} \label{AQ}
...
\newpage
\section{Programming Code for the project.}
A CD including all the programming code for the project is included on the hard copies of this paper. Nothing will appear here on the electronic copy of this paper.
